...
if ($ReportFileNameTimeStamp)
{
    ...
    $SQLFile = $ReportFilePath + "\" + $ReportFileNamePrefix + "-" + $FileTimeSuffix + ".sqlite"
}
else
{
    $ReportFile = $ReportFilePath + "\" + $ReportFileNamePrefix + ".html"
    $SQLFile = $ReportFilePath + "\" + $ReportFileNamePrefix  + ".sqlite"
}
...
#region SQL Start
$path = [System.IO.Path]::Combine($Env:TEMP,$SQLFile)
...
(
"@
$myvsphere = "freppau-hqvmmi1"; 
$sql += "'"+$myvsphere+"', ";
    $sql += "'"+$Cluster+"', ";
    $sql += "'"+$($outVMHostName)+"', ";
    $sql += "'"+$mytimestamp+"', ";
    $sql += "'"+$($vmHostRunningVmCount)+"', ";
    $sql += "'"+$($vmHostVmCount)+"', ";
    $sql += "'"+$($vmHostTotalVProc)+"', ";
    $sql += "'"+$($vmHostLpCount)+"', ";
    $sql += "'"+$consocpu+"', ";
    $sql += "'"+$(($TotalUsedMemory)[0])+"', ";
    $sql += "'"+$consomem+"', ";
    $sql += "'"+$memallocation+"', ";
    $sql += "'"+$(($TotalVisibleMemory)[0])+"'); ";
