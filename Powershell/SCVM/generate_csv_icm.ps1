...
$sUserName = "MAIN\Typhoon-hydration"
    $sPassword = ConvertTo-SecureString "@Typh00n" -AsPlainText -Force
...
write-Output " SCVMM ; Cluster ; HOST ;VM;STATE;VCPU;DynMem;MemAssigned;MemoryMaximum;SizeOfSystemFiles;os "
...
$myvmlist=Get-ClusterResource -Cluster $myclus  | Where-Object {$_.ResourceType –eq 'Virtual Machine'} | Get-VM 
  ...
  $myos = $(Get-SCVirtualMachine -VMMServer $sServer -Name $($myvm.Name) | select -expandproperty OperatingSystem)
    write-Output " $sServer;$myclus;$($myvm.ComputerName);$($myvm.Name);$($myvm.State);$($myvm.ProcessorCount);$($myvm.DynamicMemoryEnabled);$mymem_gb;$mymemmax_gb;$myvmdisk_gb;$myos "
  }
  }
} -AsJob
Wait-Job $job2
